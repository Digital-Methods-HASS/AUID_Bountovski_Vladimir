...
kings1 <- read.csv("Danskekonger.csv")

kings2 <- read_csv("Danskekonger.csv")

kings3 <- read.csv2("Danskekonger.csv")

kings4 <- read_csv2( "Danskekonger.csv")
```
Answer: 
1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object
answer 1: read_csv() and read_csv2()

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
answer 2: spec_tbl_df / tbl_df / tbl / data.frame
...
kings <- read_csv2("Danskekonger.csv")
...
```{r}
avg <- mean(kings$duration, na.rm = TRUE)
