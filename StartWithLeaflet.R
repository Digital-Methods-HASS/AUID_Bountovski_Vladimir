...
l_dk <- leaflet() %>%   # assign the base location to an object
  setView( 10.85089,55.2339084, zoom = 7) %>% 
  addTiles()
l_dk
...
# Yes, the Latitude and Longitude columns are present, and they contain numeric values in decimal degrees
...
# Your brief answer:
#Answer to task 4: Clustering is useful when there are many points, as it makes the map easier to read and less cluttered. 
# Without clustering, all points are shown individually, which can make the map messy but allows more precise location viewing.
...
saveWidget(l_dk, "DANmap.html", selfcontained = TRUE)